...
namespace NewTool
{
  ...
  public class FormatPart
  {
    ...
    public OperatorPattern FindOperator(uint value)
    {
      ...
      for (var i = 0; i < Count; i++)
      {
        ...
        if (pattern is OperatorPattern)
        {
          ...
          if ((result.Kind == OperatorKind.Around) && (value >= result.Operand) && (value <= result.Operand2))
            return result;
        }
      }
    ...
    public string GetPluralPattern(Plural plural, uint count)
    {
      ...
      if ((plural == Plural.Other) && ExistsMatching(count, out matchingPlural))
      {
  ...
  /// The English string would be "one;{0} file;other;{0} files" that contains two parts: one and other. One is for singular form and other is for plural form.
  ...
  /// becomes to 
  /// <code>"one;Item;; {0} car;other;Item;; {0} cars".</code>
  /// <para>
  /// Remember to end the resource string item name with Plural such as MyStringPlural. That way NewTool can identify the pattern as multi pattern string and 
  ...
  /// This is why you should create a resource string that contains the locale id and set that value to <see cref="MultiPattern.SetLanguage(string)"/>. 
  ...
  /// int count;
  /// ...
  /// textLabel.Text = Plural.Format("one;{0} file;other;{0} files", count);
  /// </code>
  ...
  /// int count;
  /// ...
  /// textLabel.Text = Plural.Format("zero;No files;one;{0} file;other;{0} files", count);
  /// </code>
  /// </example>
  public static class MultiPattern
  {
    private static CultureInfo culture = CultureInfo.CurrentUICulture;
    private static CultureInfo originalCulture = CultureInfo.CurrentUICulture;
    ...
    static private CultureInfo GetResourceCulture(String baseName, Assembly assembly, String resourceName, CultureInfo culture = null)
    {
    ...
    /// <param name="culture">Culture that is used as the start culture.</param>
    /// <returns>Culture info of the resource item. null if there are no localized value of the resource but only the original language independent resource.</returns>
    static private CultureInfo GetResourceCulture(ResourceManager manager, String resourceName, CultureInfo culture = null)
    {
    ...
    {
      var resourceCulture = GetResourceCulture(manager, name);

      if ((resourceCulture == null) || (resourceCulture.Name == ""))
        resourceCulture = originalCulture;

      //SetIndexProc(resourceCulture);
    ...
    /// </summary>
    /// <param name="value">Culture value</param>
    ...
    /// </summary>
    /// <param name="value">IETF language tag</param>
    ...
    /// </summary>
    /// <param name="value">IETF language tag</param>
    public static void SetOriginalLanguage(string value)
    {
      originalCulture = new CultureInfo(value);
    }
    ...
    {
      return String.Format(GetPattern(patterns, (uint)count), args);
    }
    ...
    {
      return String.Format(GetPattern(patterns, gender), args);
    }
    ...
    {
      return String.Format(GetPattern(patterns, select), args);
    }
