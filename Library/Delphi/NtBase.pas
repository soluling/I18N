...
const
  APPLICATION_RESOURCE = 'NEWTOOL';
  APPLICATION_DIR = 'NewTool';
...
{$ENDIF}
  ...
  public
    ...
    { Get a list of available resource DLL languages.
      ...
      @param dir            Specifies the directory where resource DLL exist. If empty the application directory is used.
      @return Amount of lanugages. }
    class function GetAvailable(
      languages: TNtLanguages;
      exeFileName: String = '';
      compatibleOnly: Boolean = False;
      checkVersions: Boolean = False;
      dir: String = ''): Integer;
    ...
    { Extracts embedded resource DLLs from EXE to the application directory.
      ...
      @param resourceName  Specifies the resource type name of the resource DLLs. If empty NEWTOOL is used. }
...
class function TNtBase.IsLocaleCompatible(locale: Integer): Boolean;
begin
{$IFDEF UNICODE}
  Result := True;
{$ELSE}
...
class function TNtBase.LoadNew(code: String; fileName: String): HModule;
  ...
  function Process(const fileName, ext: String): HModule;
  var
    dllFileName: String;
  begin
    if ext <> '' then
      dllFileName := ChangeFileExt(fileName, '.' + ext)
    else
...
{$ENDIF}
    ...
    begin
{$IFDEF MSWINDOWS}
      dllFileName := GetFolderPath(CSIDL_PERSONAL) + '\' + APPLICATION_DIR + '\' + ChangeFileExt(ExtractFileName(fileName), ext);
      Result := LoadLibraryEx(PChar(dllFileName), 0, LOAD_LIBRARY_AS_DATAFILE);
{$ENDIF}
    end;
  end;
    ...
    begin
      ...
      begin
        if resourceFileNameGiven then
          newInst := Process(fileName, '')
        else
        begin
          newInst := Process(fileName, code);
