...
type
  ...
  private
    ...
    procedure Process(value: Double; const long, short, currency: String);
...
procedure TNumberTests.SetLanguage(const value: String);
var
  locale: Integer;
begin
  locale := TNtLocale.ExtensionToLocale(value);
...
procedure TNumberTests.Process(value: Double; const long, short, currency: String);

  procedure Check(const actual, shouldBe: String);
  begin
    CheckEquals(StringReplace(actual, #160, ' ', [rfReplaceAll]), shouldBe);
  end;

begin
  Check(TAbbreviatedNumber.FormatLong(value, Precision), long);
  Check(TAbbreviatedNumber.FormatShort(value, Precision), short);
  Check(TAbbreviatedNumber.FormatCurrency(value, Precision), currency);
end;

procedure TNumberTests.English;
begin
  Language := 'en';
...
procedure TNumberTests.Finnish;
begin
  Language := 'fi';
...
procedure TNumberTests.Swedish;
begin
  Language := 'sv';
...
procedure TNumberTests.Japanese;
begin
  Language := 'ja';
  ...
  Process(27450, '2.75万', '2.75万', '¥2.75万');
...
initialization
  ...
  RegisterTest(TNumberTests.Suite);
end.
