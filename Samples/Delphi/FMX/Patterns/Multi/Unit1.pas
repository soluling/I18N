...
procedure TForm1.UpdateStrings;
  ...
  begin
    ...
    begin
      ...
      Result := TMultiPattern.Format(_T('I have %s;one;one ski %1:s;other;%d skis %s;next;one;and one bicycle;other;and %d bicycles', 'MessagePlural'), [ski, bicycle]);
    end
    else
    begin
      ...
      Result := TMultiPattern.Format(_T('I have %s;zero;no skis %1:s;one;one ski %1:s;other;%d skis %s;next;zero;and no bicycles;one;and one bicycle;other;and %d bicycles', 'ZeroMessagePlural'), [ski, bicycle]);
    end;

    // Note! Name the resource string id is XXXPlural. This will automatically turn on plural parsing.
  end;
