...
procedure TForm1.UpdateStrings;
  ...
  begin
    ...
    begin
      ...
      Result := TMultiPattern.Format(_T('one;%d file;other;%d files', 'MessagePlural'), count, [count]);
    end
    else
    begin
      // Contains three patterns: zero, one and other.
      Result := TMultiPattern.Format(_T('zero;No files;one;%d file;other;%d files', 'ZeroMessagePlural'), count, [count]);
    end;
